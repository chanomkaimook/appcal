...
class Bill_model extends CI_Model
{
  ...
  private function _setFilter()
  {
    ...
    $this->db->select(
      ...
      (select count(bill_items.bill_id) where bill.id = bill_items.bill_id) count_items
      "
    );
    $this->db->join('bill_items', 'bill.id = bill_items.bill_id', 'inner');
    // $this->db->join('customer', 'bill.company_id = customer.id', 'left');
    ...
    if (!empty($search['q'])) {
      ...
      $this->db->where("(
                ...
                or bill_items.product_name LIKE '%{$q}%'
            )");
    }
    ...
    if ($search && isset($search['user_start'])) {
      $this->db->where("bill.user_start", $search['user_start']);
    }

    if ($search && isset($search['code'])) {
      $this->db->where("bill.code", $search['code']);
    }
    if ($search && isset($search['date_from'])) {
      $d1    = $search['date_from'];
  ...
  public function getRecordCountWorkOrder()
  {
    ...
    $this->db->from('bill');
    $this->total_rows_workorder = $this->db->count_all_results();
    return $this->total_rows_workorder;
    // $this->db->select('count(*) as countAll');
    // $query = $this->db->get();
    // $result = $query->result_array();
    // $this->total_rows_workorder = $result[0]['countAll'];
    // return $this->total_rows_workorder;
  }
  ...
  public function getByCode($code)
  {
    ...
    $_item = $item ? array_change_key_case($item) : [];
    return $item ? array_merge($_item, [
      'items' => $this->getItemsById($item['id']),
    ]) : [];
  }
  ...
  public function getItemsById($id)
  {
    ...
    if (count($item) > 0) {
      foreach ($item as $data) {
        ...
        $data['labs']            = $this->findLabOfBill($id, $data['product_id'], $options);
  ...
  public function _listMainLab($billId = "", $productId = "")
  {
    ...
    FROM
        bill_items AS bi
        INNER JOIN product AS p ON bi.product_id = p.id
