...
class Invoice extends API_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->helper('MY_promotion_helper');
  }
  ...
  public function fetch_dt()
  {
    ...
    if ($bills) {
      ...
      foreach ($bills as $row) {
        ...
        $optional['group_by'] = array('invoice.code');
  ...
  public function list($id)
  {
    ...
    if ($id) {
      ...
      invoice.DOCUMENT_ALIAS as DOCUMENT_ALIAS,
      invoice.INVOICE_STATUS_ID as INVOICE_STATUS_ID,
      invoice.INVOICE_STATUS_CODE as INVOICE_STATUS_CODE,
      statusservice.name as STATUS_NAME,
      ';
    ...
    if ($id) {
      ...
      foreach ($items['items'] as $key => $row) {
        ...
        $optional['select'] = '
          ...
          statusservice.name as INVOICE_STATUS_NAME,
          ';
        $optional['where'] = array(
        ...
        $items['items'][$index]['invoice_id'] = $invoice_data ? $invoice_data[0]->INVOICE_ID : "";
        $items['items'][$index]['invoice_code'] = $invoice_data ? $invoice_data[0]->INVOICE_CODE : "";
        $items['items'][$index]['invoice_paid'] = $invoice_data ? $invoice_data[0]->INVOICE_PAID : "";
        $items['items'][$index]['invoice_status_name'] = $invoice_data ? $invoice_data[0]->INVOICE_STATUS_NAME : "";
